                ...
                binding.infoBtn.setOnClickListener {
                    try {
                        ...
                        if (breedsInfo.isNull(0)) {
                            ...
                            val name = if (breedsData.has(DATA.JSON_NAME)) breedsData.getString(DATA.JSON_NAME) else DATA.EMPTY
                            val origin = if (breedsData.has(DATA.JSON_ORIGIN)) breedsData.getString(DATA.JSON_ORIGIN) else DATA.EMPTY
                            val desc = if (breedsData.has(DATA.JSON_DESCRIPTION)) breedsData.getString(DATA.JSON_DESCRIPTION) else DATA.EMPTY
                            val temp = if (breedsData.has(DATA.JSON_TEMPERAMENT)) breedsData.getString(DATA.JSON_TEMPERAMENT) else DATA.EMPTY
                            val wikiUrl = if (breedsData.has(DATA.JSON_WIKIPEDIA_URL)) breedsData.getString(DATA.JSON_WIKIPEDIA_URL) else DATA.EMPTY
                            val moreLink = if (breedsData.has(DATA.JSON_VCAHOSPITALS_URL)) breedsData.getString(DATA.JSON_VCAHOSPITALS_URL) else DATA.EMPTY
